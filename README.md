...
by `GITHUB_TOKEN` starts no workflow run: the same push made with it would move all five pull requests
...
| [test/release-proposal](tests/scenario-release-proposal/README.md)<br>[PR #54](https://github.com/turboBasic/github-actions-test/pull/54) | `release-proposal.yml`'s `workflow_call` entry point, from two call sites on one push — one passing both App secrets, one withholding `app-private-key` | 💚 **and** ❤️ together. The first opened #53, `chore: release v0.2.1`, diff exactly `pyproject.toml` and `uv.lock`. The second is `startup_failure` with **zero jobs, zero logs and zero check runs** — a withheld required secret refuses the run as an ungranted permission does, which is why the capability declares its secrets instead of taking `secrets: inherit`. `0.2.1` and not `0.3.0` because below `1.0.0` the minor is the break axis. |
